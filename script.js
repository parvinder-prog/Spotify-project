...
// Fetch MP3 files from a folder and list them
...
const playMusic = (track, pause = false) => {
    ...
    currentIndex = songs.indexOf(track);  // Update currentIndex here
    if (!pause) {
        currentSong.play();
...
async function displayAlbums() {
    ...
    for (const a of anchors) {
        if (a.href.includes("/songs/")) {
            ...
            if (folder) {
                try {
                    let infoRes = await fetch(`songs/karan aujla/info.json`);
    ...
    document.querySelectorAll(".card").forEach(card => {
        card.addEventListener("click", async () => {
            ...
            if (songs.length > 0) {
                playMusic(songs[0]);  // play first song when card clicked
            }
        });
    });
}
...
async function main() {
    ...
    // Display album cards (no default song play)
    await displayAlbums();
